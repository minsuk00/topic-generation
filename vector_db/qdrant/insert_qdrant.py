...
def get_all_json_files(directory: str = "vectors"):
    ...
    while retry_count < 3:
        try:
            client.upsert(
                ...
                points=Batch(
                    ids=[int(vec["shorten_id"]) for vec in batch],
                    vectors=[vec["vector"][0] for vec in batch],
                    payloads=[vec["payloads"] for vec in batch],
                ),
            )
            return
